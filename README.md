...
**It reads harmony from one instrument at a time.** A student playing in the
room, a solo guitar demo, a sparse recording: the chord chart is usually worth
handing over. A produced track - full band, vocals, heavy effects - comes back
with the wrong chords, and it is not close.

That is not a caveat bolted on after the fact; it is the measured behaviour, and
for a long time this README implied otherwise. The accuracy numbers below come
from audio generated by `testAudio.ts`, which is built from the same model the
detector assumes: exact equal temperament, chord tones restated on every beat, a
clean root in the bass. **It is a circular benchmark.** Degrading it on purpose
barely moves the result:

| Synthetic clip | Bars correct |
| --- | --- |
| clean guitar only | 100% |
| + a vocal line | 92% |
| + cymbal wash | 100% |
| + heavy distortion (12 partials) | 100% |
| all three at once | 93% |

A real record scores far worse than the worst row there, because the ways it
differs are ones this synthesis cannot fake: inharmonic instruments, a bass line
that walks away from the root, chords that ring rather than restrike, a lead
vocal moving through non-chord tones, and slash chords the vocabulary does not
contain at all.

Two specific weaknesses are worth naming, because they are in the code rather
than in the physics:

- In `chordEngine.ts` the per-frame score is
  `cosine(frame, template) * 6 + keyBias + bassTerm`. The bass term swings
  ±0.44/0.66; the key bias is ±0.05 and `engine.ts` never overrides the default.
  **The bass term outweighs the key by roughly 13 to 1**, so a moving bass line
  names the chord and the detected key is effectively decorative.
- The melody tracker's default range is MIDI 52-84 (E3 to C6), which is the
  whole lead-vocal range, and `trackPitch` is monophonic. On anything with
  singing on it, the "riff" is a trace of the vocal.

Rather than paper over this, the app measures its own output
(`src/analysis/reliability.ts`): a progression that never settles into a key, or
that names a new chord almost every bar, gets a banner above the tab saying so.
The verdict is recomputed from the arrangement, so correcting bars by hand makes
it go away.
...
| --- | --- |
| Effects and layers add phantom notes | Overstated. The chord engine survives synthetic density and not a real mix; see **What it can and cannot do**. |
| Chord clusters come back messy | Everything snaps to the nearest common voicing, as a suggestion you can override per bar. |
| Octave and register leakage (bass, vocals) | Separate pitch-range filters for harmony and melody, plus 8va/8vb buttons per note in the riff editor. The melody default still spans the vocal range, so on a sung track it follows the voice. |
| Ambiguous chords (add9, sus, slash) | The plain triad is shown by default, with "use detected *Xadd9*" one click away. **Slash chords are not in the vocabulary at all** - a G/F# is named after the F#. |
| Tempo half/double errors | Flagged when the two trackers disagree; ×2 and ÷2 buttons sit next to the tempo field. |
| The recording is off concert pitch | Estimated and reported **only on the built-in chroma fallback**. On the Essentia path (the normal one) `tuningOffset` stays 0 and the warning never fires. |
...
60 tests. The pipeline tests synthesise chord progressions, run the real
analysis path, and assert the chart that comes out is the one a teacher would
write down - 5/5 progressions recovered exactly, with tempo inside 5%. Read that
number with **What it can and cannot do** open: the audio is generated from the
detector's own assumptions, so it measures that the code does what it intends,
not that the intent survives a record.

The exporter tests check wellformedness, the invariants MuseScore and Guitar Pro
actually care about, and the character alignment the tab grid and its playhead
depend on. The reliability tests pin the verdict against both a clean
progression and the real scrambled output from a run on a dense rock mix.
